...
export default function ReportCard({ template, onGenerate, onPreview }) {
  return (
    <Box sx={{ p: 3, borderRadius: radius.xl, bgcolor: semantic.paper, border: `1px solid ${semantic.border}`, transition: "all 0.25s ease", display: "flex", flexDirection: "column", height: "100%", "&:hover": { boxShadow: shadows.md, transform: "translateY(-3px)", borderColor: `${semantic.info}40` } }}>
      ...
      <Stack direction="row" spacing={1}>
        ...
        <Tooltip title="Export"><IconButton size="small" sx={{ border: `1px solid ${semantic.border}`, borderRadius: radius.md, width: 34, height: 34 }}><Download sx={{ fontSize: 15, color: semantic.textTertiary }} /></IconButton></Tooltip>
      </Stack>
    </Box>
  );
}
