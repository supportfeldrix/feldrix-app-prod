/**
 * Feldrix Control Centre — Farms Page (Placeholder)
 * Sprint 46.2
 */
...
export default function AdminFarms() {
  return (
    <Stack spacing={3}>
      <Box>
        <Typography sx={{ fontSize: "1.5rem", fontWeight: 800, color: ADMIN_THEME.text }}>Farms</Typography>
        <Typography sx={{ fontSize: "0.9rem", color: ADMIN_THEME.textSecondary }}>Browse all farms on the platform.</Typography>
      </Box>
      <Box sx={{ p: 4, borderRadius: 3, bgcolor: "#fff", border: `1px dashed ${ADMIN_THEME.primary}30`, textAlign: "center" }}>
        ...
        <Stack spacing={1}>
          {["Farm Table (name, owner, location, animals, crops)", "Farm Health Score", "Last Activity", "Farm Detail Drawer"].map((s) => (
